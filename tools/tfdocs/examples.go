...
// to fix.
//
// An example is rewritten on every run, so it follows the schema rather than the
// schema it was first made from: the day the API moved a tag's fields into a
// nested `config`, an example left alone would have gone on showing the old
// arguments, the documentation would have embedded them, and every check would
// have passed. A file that does not open with generatedMarker is somebody else's
// and is never touched, which is how an object worth explaining properly gets
// explained properly while the generator fills in the long tail.
//
// What reaches a documentation page is the configuration alone: `tfdocs strip`
// takes the marker back out of the block tfplugindocs embedded the example into.
const (
...
// generatedMarker is the first line of every example this generator owns, and the
// whole of how it recognises its own work on the next run. An example without it
// belongs to whoever wrote it and is never rewritten, however far the schema has
// moved: that is the escape hatch for the objects a minimal example does not
// serve, and the provider's own example — a `terraform` block, which is not a
// shape any object's schema has — is the one that uses it.
//
// It is one line, in the comment syntax of both kinds of example, and it says what
// deleting it does, because deleting it is the decision it exists to offer. It does
// not reach the documentation: `tfdocs strip` takes it back out of the block it was
// embedded into, so the code a practitioner copies off a page is configuration and
// nothing else.
const generatedMarker = "# generated by tools/tfdocs; delete this line to maintain the example by hand"
...
// writeExample writes an example, unless the one on disk is somebody's own.
//
// A file whose first line is the marker is the generator's, and is rewritten so
// that it follows the schema it was made from. A file without it is left alone.
func writeExample(path, content string) (outcome, error) {
	...
	if err == nil && !strings.HasPrefix(string(existing), generatedMarker) {
		return kept, nil
	}
